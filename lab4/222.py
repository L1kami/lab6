...
def main():
    ...
    Створює об'єкти, виводить їх характеристики, розраховує
    середню потужність і виводить список пилок, потужніших за середнє.
    """
    ...
    # --- Початковий вивід (залишаємо для демонстрації) ---
    ...
    average_power = 0.0
    if saws:
        total_power = sum(saw.get_power_watts() for saw in saws)
        average_power = total_power / len(saws)
        print("--- Розрахунок середньої потужності ---")
        print(f"Загальна потужність (3 пилок): {total_power:.2f} Вт")
        print(f"Середня потужність: {average_power:.2f} Вт\n")
    else:
        print("Немає пилок для розрахунку.\n")

    # --- Вивід пилок, потужніших за середнє ---
    if average_power > 0:
        print(f"--- Пилки, потужніші за середнє ({average_power:.2f} Вт) ---")
        found_powerful = False
        for saw in saws:
            if saw.get_power_watts() > average_power:
                print(f"  -> {saw.get_name()} (Потужність: {saw.get_power_watts()} Вт)")
                found_powerful = True

        if not found_powerful:
            print("  (Не знайдено пилок, потужніших за середнє)")
